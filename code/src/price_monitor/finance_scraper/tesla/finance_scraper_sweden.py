...
from src.price_monitor.finance_scraper.tesla.finance_parser_sweden import (
    parse_sweden_finance_line_items,
)
from src.price_monitor.finance_scraper.tesla.selenium_sweden import (
    get_finance_details_for_sweden_model,
)
...
class FinanceScraperTeslaSweden:
    def __init__(
        self,
    ...
    def scrape_finance_options_for_sweden(self):
        response = []

        models = _find_available_models(market=Market.SE)
    ...
    def scrape_finance_option_for_model(self, model: str) -> List[FinanceLineItem]:
        ...
        try:
            ...
            finance_line_details = get_finance_details_for_sweden_model(url)
            response = parse_sweden_finance_line_items(line_items, finance_line_details)
