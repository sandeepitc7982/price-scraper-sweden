...
class TeslaFinanceScraper(FinanceVendorScraper):
    def __init__(
        self,
        ...
        self.markets = config["finance_scraper"]["enabled"][Vendor.TESLA]
        self.config = config
    ...
    def scrape_finance_options(self, market: Market) -> List[FinanceLineItem]:
        ...
        try:
            ...
            logger.error(
                f"[{market}] Failed to scrape finance options for {Vendor.TESLA}, {e}. Loading previous dataset..."
            )
            response.extend(
    ...
    def _scrape_finance_options_for_market(
        ...
        response: List[FinanceLineItem] = []
        if market == Market.UK:
            finance_scraper_uk = FinanceScraperTeslaUk(
